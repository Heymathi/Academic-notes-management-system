...
class NotesOrganizer {
    ...
    attachEventListeners() {
        ...
        if (this.uploadSubjectSelect) {
            this.uploadSubjectSelect.addEventListener('change', () => {
                console.log('Upload subject changed:', this.uploadSubjectSelect.value);
            });
        }
    ...
    renderFiles() {
        ...
        // Render files organized by folder
    ...
    filterSubjectsFiles(query) {
        ...
        if (!this.currentSubject) {
            this.filesList.innerHTML = '<div class="empty-files-message"><p>Select a subject first.</p></div>';
            this.notesList.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: #999;">Select a subject first.</div>';
            return;
        }
        if (!q) {
            this.renderFiles();
        ...
        // Search within current subject for ANY single character match
